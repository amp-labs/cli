...
func Get() (Config, error) {
	...
	if err := json.Unmarshal(data, &c); err != nil {
		return Config{}, fmt.Errorf("can't parse config: %v", err)
	}
...
func Set(config Config) error {
	...
	if err := mergo.Merge(&merged, existing); err != nil {
		return fmt.Errorf("can't merge new config with existing config: %v", err)
	}
...
func writeFile(path string, data []byte) error {
	return os.WriteFile(
		path,
		data,
		0600, // Regular file with read/write permission for owner
	)
}
